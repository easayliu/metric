...
use metricpipe::sink::{ClickhouseSink, Sink};
...
async fn insert_body_is_gzipped_and_round_trips() {
    ...
    let mut sink =
        ClickhouseSink::new(endpoint, "logs", "otel_metric").timeout(Duration::from_secs(5));
    ...
    assert!(
        plain.contains("\"metric_name\":\"压缩测试\""),
        "解压后对不上: {plain}"
    );
    assert!(
        plain.ends_with('\n'),
    ...
    assert!(
        plain.contains("\"exemplars.timestamp\":[\"2026-09-07 03:04:08.914293457+00:00\"]"),
        "{plain}"
    );
}
...
async fn compression_can_be_turned_off() {
    ...
    assert!(String::from_utf8_lossy(&body).contains("\"metric_name\":\"压缩测试\""));
}
...
async fn timezone_puts_wall_clock_and_offset_on_timestamps() {
    ...
    assert!(
        body.contains("\"start_timestamp\":\"2026-09-07 11:03:08.914293456+08:00\""),
        "{body}"
    );
    assert!(
        body.contains("\"exemplars.timestamp\":[\"2026-09-07 11:04:08.914293457+08:00\"]"),
        "{body}"
    );
}
